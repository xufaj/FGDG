...
client.on('message', async message => {
  ...
  if(message.content.startsWith(prefix + "mute")) {
    ...
    let mention = message.mentions.members.first();//حقوق DEAD و SAJED
    if(!mention) return  message.channel.send('').then(msg => { //حقوق DEAD و SAJED
      msg.delete(3500);
      message.delete(3500);
    });
    ...
    if(mention.id === message.author.id) return message.channel.send('**:x:You Cannot give mute to your self**').then(msg => {
      msg.delete(3500);
      message.delete(3500); //حقوق DEAD و SAJED
    });
   
    if(mention.hasPermission('ADMINISTRATOR')) return message.channel.send(`**:x: لا يمكن آعطاء ميوت لادارة السيرفر**`); //حقوق DEAD و SAJED
    ...
    .addField('**__السيرفر__**',[ message.guild.name ]) //حقوق DEAD و SAJED
    ...
    if(!role) try {
      message.guild.createRole({
        name: "Muted",
        permissions: 0 //حقوق DEAD و SAJED
      }).then(r => {
        message.guild.channels.forEach(c => {
          c.overwritePermissions(r , {
            SEND_MESSAGES: false, //حقوق DEAD و SAJED
    ...
    mention.addRole(role).then(() => {
      ...
      mention.setMute(true); //حقوق DEAD و SAJED
    });
    setTimeout(() => {
      if(duration === 0) return;
      mention.setMute(false);
      mention.removeRole(role)
    },duration * 60000); //حقوق DEAD و SAJED
  }
});
client.on('message', async message => {
...
let command = message.content.split(" ")[0];
     command = command.slice(prefix.length);
    let args = message.content.split(" ").slice(1);  //حقوق DEAD و SAJED
if(command === `unmute`) {2
...
if(!message.guild.member(client.user).hasPermission("MUTE_MEMBERS")) return message.reply("**I donot Have Permission Mute_Members**").then(msg => msg.delete(6000))
  ...
  await kinggamer.removeRole(role) //حقوق DEAD و SAJED
...
client.on("message", (message) => {
    ///حقوق DEAD و SAJED
   if (message.content.startsWith("$new")) {     /// حقوق DEAD و SAJED
        const reason = message.content.split(" ").slice(1).join(" ");     /// حقوق DEAD و SAJED
        if (!message.guild.roles.exists("name", "Support Team")) return message.channel.send(`لازم تسوي رتبة اسمها \`Support Team\` وتنطي البوت ادمنيتر حتا يقدر يسوي الرومات ويعدل برمشنات`);
        if (message.guild.channels.exists("name", "ticket-{message.author.id}" + message.author.id)) return message.channel.send(`You already have a ticket open.`);    /// SAJED...👻#8844 
...
client.on('ready', () => {
   console.log(`----------------`);
      console.log(`RIVER Bot- Script By : SAJED`);
        console.log(`----------------`);
      console.log(`ON ${client.guilds.size} Servers '     Script By : EX Clan ' `);
    console.log(`----------------`);
  console.log(`Logged in as ${client.user.tag}!`);
client.user.setGame(`$help | TOKYO System`,"http://twitch.tv/DJ")
